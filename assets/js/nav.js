...
var $gnb_dep2_item = $gnb_dep2.children("ul").children("li").children("a");
...
function openEachMenu () {
	$gnbItem.children("a").on("mouseenter focus",function  () {
		...
		$gnb_dep2_item.off("mouseenter focus").on("mouseenter focus", function() {
			if ($(this).siblings(".gnb-3dep").length) {
				$(this).addClass('open-3dep');
				$(this).siblings(".gnb-3dep").addClass("open");
			}
		});

		$gnb_dep2.children("ul").children("li").off("mouseleave").on("mouseleave", function() {
			$(this).find('a').removeClass('open-3dep');
			$(this).find(".gnb-3dep").removeClass("open");
		});
	})
	...
	function gnb_return () {
		...
		$gnb_dep2.children("ul").children("li").removeClass('open-3dep');
		$(".gnb-3dep").removeClass("open");
	}
}
...
function calcMenuHeight () {
	if ( $.exists(".header-util-menu-box") &&  !($.exists(".gnb-mode-full"))  ){
		$(".gnb-navigation-wrapper").height(getWindowHeight() - $(".header-util-menu-box").height());
	}
}
//  gnbM full스타일 클래스 지정
function addClassFullMenu () {
	if ( $.exists(".gnb-mode-full") && $.exists(".member-menu-box") ) {
		$(".gnb-mode-full").addClass("gnb-mode-full-member");
	}
}
