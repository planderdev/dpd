...
판별 함수 `private.is_admin()` 은 API 에 노출되지 않는 스키마에 있습니다.
...
2026-09-17, DPD 조직의 프로젝트 `kxiavmomonkwohjefybs` 에 적용 완료했습니다.

확인한 내용

- 테이블 15개 생성, 시드 98건 입력
- 입력한 13개 테이블 전부 원본 데이터와 행별 해시가 일치
- 보안 어드바이저 경고 0건
- 익명 권한으로 실제 접근을 확인
  - 노출 콘텐츠 읽기 O, 문의 접수 목록 읽기 X, 관리자 목록 읽기 X
  - 공지 등록 X, 히어로 수정 X
  - 문의 접수 O, 처리 상태·관리자 메모 위조 X
